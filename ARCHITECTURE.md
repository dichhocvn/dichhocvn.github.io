...
- Theme/template + API ảnh lyso.vn (khóa config thủ công, ẩn legend/prompt/`#luuModeBar` khi ở mode API, URL ảnh có năm xem hạn từ `namXem`; ảnh nền ô trung tâm CFG từ `resources/background-laso/` theo `LASO_BG_FILES` → `--laso-center-bg` (URL tuyệt đối, tránh `url()` resolve theo `grid.css` khi `file://`) / `.center::before`, tuỳ chọn Âm Dương SVG (`laso-center-decor-yinyang`), đồng bộ khi đổi template khỏi lyso) -> `src/js/features/theme-config.js` + `src/css/grid.css`; SVG trang trí ô giữa -> `tuvi-render-modules.js` (`renderCenter`)
