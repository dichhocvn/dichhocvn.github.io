...
- Tinh toan la so tu input (router method) -> `src/js/core/logic.js`; shared core -> `src/js/core/ansao.js`; method modules -> `src/js/core/trungchauphai.js`, `src/js/core/thaithulang.js` (TTL da ap dung cac bang sao theo gio/nam/thang/can dang index da xu ly san + Tu Hoa + M/V/Đ/B/H; Tuan/Triet giong Trung Chau; phan con lai dung shared core; `Lưu Hà` theo can nam: Giap-Dau, At-Tuat, Binh-Mui, Dinh-Than, Mau-Ty, Ky-Ngo, Canh-Mao, Tan-Thin, Nham-Hoi, Quy-Dan)
- Tien ich dung chung (dia chi/thien can/cung chuc) -> `src/js/utils/diachi.js`, `src/js/utils/thiencan.js`, `src/js/utils/cungchuc.js`
- Bang tra sao, du lieu co dinh (common) -> `src/js/core/data.js`; bang rieng tung method (vd Trung Chau: Tu Hoa/Mieu Vuong/Sao theo thang/Sao hanh...) -> file method tuong ung
