...
- Theme/config: `src/js/features/theme-config.js` (template `lyso.vn` khóa chỉnh config thủ công, render ảnh API trong vùng lá số, URL ảnh lấy thêm năm xem hạn hiện tại; ẩn `#luuModeBar` vì ảnh API không theo chế độ an sao lưu)
