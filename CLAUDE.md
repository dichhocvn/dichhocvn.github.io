...
## Route task -> file
- Tính lá số từ input: `src/js/core/logic.js` (router method); shared core: `src/js/core/ansao.js`; method modules: `src/js/core/trungchauphai.js`, `src/js/core/thaithulang.js` (TTL đã áp bảng sao theo giờ/năm/tháng/can dạng index xử lý sẵn + Tứ Hóa + M/V/Đ/B/H; Tuần/Triệt giữ giống Trung Châu; phần còn lại dùng shared core; `Lưu Hà` theo can năm: Giáp-Dậu, Ất-Tuất, Bính-Mùi, Đinh-Thân, Mậu-Tý, Kỷ-Ngọ, Canh-Mão, Tân-Thìn, Nhâm-Hợi, Quý-Dần)
- Tiện ích dùng lại nhiều (địa chi/thiên can/cung chức): `src/js/utils/diachi.js`, `src/js/utils/thiencan.js`, `src/js/utils/cungchuc.js`
- Bảng tra dữ liệu sao: `src/js/core/data.js` (common); dữ liệu riêng từng phương pháp đặt ở file method tương ứng (`trungchauphai.js` / `thaithulang.js`)
