...
- Theme/config: `src/js/features/theme-config.js` (template `lyso.vn` khóa chỉnh config thủ công, render ảnh API trong vùng lá số, URL ảnh lấy thêm năm xem hạn hiện tại; ẩn `#luuModeBar` vì ảnh API không theo chế độ an sao lưu; ảnh nền CFG từ `resources/background-laso/` + `LASO_BG_FILES` (option hiển thị tên không đuôi; Không nền / Âm Dương vector / file ảnh) → `--laso-center-bg` (URL tuyệt đối để tránh resolve `url()` theo file CSS khi mở `file://`) + `laso-center-wallpaper-on` hoặc `laso-center-decor-yinyang` trên `#lasoWrap`; không áp khi khóa lyso API)
