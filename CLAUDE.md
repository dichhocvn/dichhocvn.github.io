...
- Responsive/layout: `src/js/features/layout-scale.js`, `layout-responsive.js`, `layout-resize.js`; mobile: `form-mobile-pickers.js` + `forms.css` (select ngày/tháng/giờ; reserve top `--laso-overflow-top-reserve` để khi nhiều sao thì lá số dồn lên trên)

## CSS modules
- `src/css/base.css`: variables + reset + shell layout (toàn page padding 2px)
- `src/css/chat.css`: chatbot UI
- `src/css/forms.css`: form + tab + wrapper + search nav class-based responsive + margin-bottom 2px cho grid mobile + tab bar sticky desktop khi cuộn + reserve top `--laso-overflow-top-reserve`
